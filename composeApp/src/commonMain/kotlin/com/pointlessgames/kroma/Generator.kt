...
internal object Generator {
    ...
    private suspend fun internalGenerate(width: Int, height: Int): LevelData? = runCatching {
        ...
        repeat(MAX_GENERATIONS) { generation ->
            ...
            if (bestInGeneration.second >= complexity) {
                ...
                return@runCatching pruneUntouchedTiles(bestInGeneration.first)
            }
            ...
            while (currentCoroutineContext().isActive && nextGeneration.size < POPULATION_SIZE) {
                ...
                var parent1 = scoredPopulation.take(POPULATION_SIZE / 2).random(random).first
        ...
        return@runCatching bestOverall?.takeIf { it.second > 0 }?.first?.let {
            pruneUntouchedTiles(
                it
            )
        }
